...
In this chapter, we'll define the shared components, messages, and protocol that both the client and server will use. This is the foundation of our multiplayer snake game.

## Game Components

Let's start by defining the core game components in `shared/src/lib.rs`:
...
Now let's define the messages that clients send to the server:
...
## Lightyear Protocol Setup

Lightyear requires us to define a protocol for networking. Add this to the shared crate:
...
impl ProtocolSet for SnakeProtocol {
    fn build(builder: &mut ProtocolBuilder) {
        // Register components that will be replicated
        ...
        builder.register_component::<GameState>(ChannelKind::Reliable);

        // Register messages
        builder.register_message::<InputMessage>(ChannelKind::Unreliable);
        builder.register_message::<ServerMessage>(ChannelKind::Reliable);
    }
}

// Protocol configuration
pub fn build_client_protocol() -> ClientProtocol {
    let mut builder = ProtocolBuilder::new();
    SnakeProtocol::build(&mut builder);
    builder.build_client()
}

pub fn build_server_protocol() -> ServerProtocol {
    let mut builder = ProtocolBuilder::new();
    SnakeProtocol::build(&mut builder);
...
Let's add some utility functions for game logic:
...
Finally, let's add some resource types that we'll use:
...
## Complete shared/src/lib.rs

Here's the complete file with all components organized:
...
// Components (Snake, Food, Player, GameState implementations here)
// Messages (InputMessage, ServerMessage enums here)
// Protocol (SnakeProtocol implementation here)
// Utils (utility functions here)
// Resources (timers and config here)
```

## Testing the Shared Crate

Make sure everything compiles:

```bash
cargo check -p snake_shared
```

## What's Next?

Now that we have our shared components and protocol defined, we can move on to implementing the [Server Implementation](./server.md). The server will use these components to manage the authoritative game state, process player input, and handle game logic like collision detection and food spawning.