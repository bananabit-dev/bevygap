...
pub async fn list_rooms<T: HasLobby>(State(state): State<Arc<T>>) -> Json<Vec<LobbyRoom>> {
...
pub async fn create_room<T: HasLobby>(State(state): State<Arc<T>>, Json(req): Json<CreateRoomRequest>) -> Result<Json<LobbyRoom>, (axum::http::StatusCode, String)> {
...
pub async fn lobby_status<T: HasLobby>(State(state): State<Arc<T>>) -> Json<LobbyStatus> {
    let rooms = state.lobby().rooms.lock().unwrap();
    let total = rooms.len();
...
pub async fn start_room<T: HasLobby>(State(state): State<Arc<T>>, Path(id): Path<String>) -> Result<(), (axum::http::StatusCode, String)> {
    let mut rooms = state.lobby().rooms.lock().unwrap();
...
pub async fn leave_room<T: HasLobby>(State(state): State<Arc<T>>, Path(id): Path<String>, Json(_req): Json<LeaveRoomRequest>) -> Result<StatusCode, (StatusCode, String)> {
    let mut rooms = state.lobby().rooms.lock().unwrap();
    if let Some(room) = rooms.get_mut(&id) {
