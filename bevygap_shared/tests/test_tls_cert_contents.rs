use bevygap_shared::nats::BevygapNats;

#[tokio::test]
async fn test_tls_certificate_contents_parsing() {
    // Test that the TLS certificate parsing logic works correctly
    let test_ca_contents = TEST_CERT;
    
    // Set up environment variables
    std::env::set_var("NATS_CA_CONTENTS", test_ca_contents);
    std::env::set_var("NATS_HOST", "invalid-host-for-testing.local:4222");
    std::env::set_var("NATS_USER", "test");
    std::env::set_var("NATS_PASSWORD", "test");
    
    // Try to create a connection (it will fail due to invalid host, but should parse certificates correctly)
    let result = BevygapNats::new_and_connect("test_tls_cert_parsing").await;
    
    // We expect this to fail due to the invalid host, but the error should not be about certificate parsing
    assert!(result.is_err());
    let error_msg = result.unwrap_err().to_string();
    
    // The error should be about connection, not certificate parsing
    assert!(!error_msg.contains("certificate"), "Error should not be about certificates: {}", error_msg);
    
    // Clean up
    std::env::remove_var("NATS_CA_CONTENTS");
    std::env::remove_var("NATS_HOST");
    std::env::remove_var("NATS_USER"); 
    std::env::remove_var("NATS_PASSWORD");
}

const TEST_CERT: &str = r#"-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"#;